...
/**
 * <p>
 * This class implements the {@link INcdReportRepositoryBranchCommitCollector}
 ...
 * 
 * <p>
 * The {@link NcdReportContext} uses this class for collecting and
 * processing commit data as follows:
 * </p>
 * <ul>
 * <li>Creating a new instance of this class for every individual repository
 * being processed</li>
 * <li>Passing this instance to the various
 * {@link INcdReportBranchCommitGenerator} instances
 * used to generate the commit data</li>
 * <li>Calling the {@link #writeResults(NcdReportResultsWriters)} method once
 * all commit data
 * for the current repository has been generated</li>
 * <li>Invoke the various getters to retrieve collected data for further
 * processing</li>
 * </ul>
 * 
 * @author rsenden
...
final class NcdReportRepositoryBranchCommitCollector implements INcdReportRepositoryBranchCommitCollector {
    ...
    public void reportBranchCommit(NcdReportBranchCommitDescriptor branchCommitDescriptor) {
        if (branchCommitDescriptor.getRepositoryDescriptor() != repositoryDescriptor) {
            throw new FcliBugException(String.format(
                    "Non-matching repository descriptor; please submit an fcli bug\n\trepositoryDescriptor: %s\n\tbranchCommitDescriptor.repositoryDescriptor: %s",
                    repositoryDescriptor, branchCommitDescriptor.getRepositoryDescriptor()));
        }
        var authorDescriptor = authorCollector.reportAuthor(branchCommitDescriptor.getAuthorDescriptor(),
                branchCommitDescriptor.getCommitDescriptor().getDate());
    ...
    void writeResults(NcdReportResultsWriters writers) {
        branchCommitDescriptors.forEach((commitDescriptor, authorDescriptor) -> writers.commitsByBranchWriter()
                .writeBranchCommit(commitDescriptor, authorDescriptor));
        commitDescriptors.forEach((commitDescriptor, authorDescriptor) -> writers.commitsByRepositoryWriter()
                .writeRepositoryCommit(repositoryDescriptor, commitDescriptor, authorDescriptor));
        authorDescriptors.forEach(authorDescriptor -> writers.authorsByRepositoryWriter()
                .writeRepositoryAuthor(repositoryDescriptor, authorDescriptor));
    }
